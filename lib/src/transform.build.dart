...
class VuedartBuildContext {
   ...
   String methodParams(List<FormalParameter> params, {bool isDecl = false}) =>
      params
         .map((p) => p is DefaultFormalParameter && isDecl
   ...
   String codegenModel(Model model) =>
      model != null
         ? 'new VueModel("${model.prop}", ${codegenModelEvent(model.event)})'
         : null;
...
$opts
   ...
   List<UriReferencedElement> getUrisUsedByLibrary(LibraryElement lib) =>
      <UriReferencedElement>[]..addAll(lib.imports)..addAll(lib.exports);
